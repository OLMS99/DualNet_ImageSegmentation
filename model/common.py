...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv1 = deconv3x3(in_planes, planes, stride)
        ...
        if stride != 1 or in_planes != self.expansion * planes:
            self.shortcut = nn.Sequential(
                nn.ConvTranspose2d(in_planes, self.expansion * planes, kernel_size=1,
...
class MaskNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, nf):
        ...
        self.f_conv1 = self._make_deconv2d_layer(3, nf, padding = 1)
        self.f_conv2 = self._make_deconv2d_layer(nf*1, nf*2, padding = 1)
        self.f_conv3 = self._make_deconv2d_layer(nf*2, nf*4, padding = 1)
        self.f_conv4 = self._make_deconv2d_layer(nf*4, nf*8, padding = 1)
        self.relu = nn.ReLU()
        self.classifier = conv1x1(3, nf * 1)
    ...
    @staticmethod
    def _make_deconv2d_layer(in_maps, out_maps, padding = 1):
        layers = [nn.ConvTranspose2d(in_maps, out_maps, kernel_size=3, stride=1, padding=padding),
                nn.BatchNorm2d(out_maps),nn.ReLU()]
        return nn.Sequential(*layers)
